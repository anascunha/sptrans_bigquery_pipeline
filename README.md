...
GTFS: Baixe ZIP completo → extraia em /data/gtfs/ (agency.txt, routes.txt, ...).

Passo a Passo (Atualizado)
1. Crie Tabelas Bronze (BigQuery Console - uma vez)
Rode ingest/create_tables.sql ou cole:
sql-- Posições, Linhas, Paradas (OLHO VIVO)
...
bashpython ingest/ingest_gtfs.py
...
Scheduler: Cloud Scheduler → Cloud Run / Functions (free tier).